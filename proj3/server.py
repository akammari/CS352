...
def get(requestheaders, session, root_directory):
    ...
    if not sessionID or sessionID not in session:
        log(f"COOKIE INVALID: {requesttarget}")
    ...
    if (datetime.datetime.now() - timestamp).seconds > SESSION_TIMEOUT:
        log(f"SESSION EXPIRED: {username} : {requesttarget}")
        return "401 Unauthorized", ""
...
def start(ip, port, accounts_file, session_timeout, root_directory):
    ...
    while True:
        ...
        headerdict = {"Method": method, "Request-Target": requesttarget}
        ...
        responseheaders = f"HTTP/1.0 {status}\n\n"
        clientsocket.sendall((responseheaders + response).encode("utf-8"))
        clientsocket.close()
