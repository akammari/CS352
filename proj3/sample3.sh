...
curl --http1.0 -i -v -X POST -H "password: somepassword" "http://${SERVER_IP}:${SERVER_PORT}/" 

echo ""

# Test Case 2: No Password (POST at the root)
echo "Test Case 2: No Password"
curl --http1.0 -i -v -X POST -H "username: someuser" "http://${SERVER_IP}:${SERVER_PORT}/" 

echo ""

# Test Case 3: Username Incorrect (POST at the root)
echo "Test Case 3: Username Incorrect"
curl --http1.0 -i -v -X POST -H "username: wronguser" -H "password: 4W61E0D8P37GLLX" "http://${SERVER_IP}:${SERVER_PORT}/" 

echo ""

# Test Case 4: Password Incorrect (POST at the root)
echo "Test Case 4: Password Incorrect"
curl --http1.0 -i -v -X POST -H "username: Jerry" -H "password: wrongpassword" "http://${SERVER_IP}:${SERVER_PORT}/" 
...
SESSION_COOKIE=$(curl --http1.0 -i -v -X POST -H "username: Jerry" -H "password: 4W61E0D8P37GLLX" "http://${SERVER_IP}:${SERVER_PORT}/" | grep -i 'Set-Cookie' | cut -d ' ' -f 2 | cut -d '=' -f 2)

echo ""

echo "Cookie (sessionID) for user 1: ${SESSION_COOKIE}"
...
echo "New Cookie (sessionID) for user 1: ${SESSION_COOKIE}"

echo ""

# Test Case 7: Invalid Cookie (GET)
echo "Test Case 7: Invalid Cookie"
curl --http1.0 -v -X GET -H "Cookie: sessionID=invalidcookie" "http://${SERVER_IP}:${SERVER_PORT}/file.txt"
...
curl --http1.0 -v -X GET -H "Cookie: sessionID=$SESSION_COOKIE" "http://${SERVER_IP}:${SERVER_PORT}/file.txt"

echo ""

# Test Case 9: Username (2nd username) correct/password correct (POST)
echo "Test Case 9: Correct Username and Password for user 2"
SESSION_COOKIE=$(curl --http1.0 -i -v -X POST -H "username: Matthew" -H "password: 51HCPUQXYYQHNAI" "http://${SERVER_IP}:${SERVER_PORT}/" | grep -i 'Set-Cookie' | cut -d ' ' -f 2 | cut -d '=' -f 2) 

echo ""

echo "Cookie (sessionID) for user 2: ${SESSION_COOKIE}"
...
curl --http1.0 -v -X GET -H "Cookie: sessionID=$SESSION_COOKIE" "http://${SERVER_IP}:${SERVER_PORT}/nonexistentfile.txt"
